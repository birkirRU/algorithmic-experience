...
class DFA:
    ...
    def min_word_length(self) -> int:
        """
        Finds the shortest length to any final state, using bfs
        Precondition: non-empty language
        """

        transition_queue = []
        transition_queue.append(self.initial_state)
        visited = set()
        distTo: dict[tuple, int] = {key: math.inf for key in self.transitions}
        distTo[tuple(self.initial_state)] = 0
        ...
        while transition_queue:
            ...
            for trans in self.transitions[key]:
                ...
                if distTo[key] + 1 < distTo[next_key]:
                    distTo[next_key] = distTo[key] + 1
                    transition_queue.append(next_key)
                

        reachable_final_states_lengths = []
        for fs in self.final_states:
            rfs_l = distTo.get(fs)
            if rfs_l is None:
                continue
            else:
                reachable_final_states_lengths.append(rfs_l)

        return min(reachable_final_states_lengths)
