...
def main():
    ...
    dfa1.read_dfa()
    dfa2.read_dfa(start_state=dfa1.n)
    ...
    nfa.initial_state = dfa1.initial_state


    for state1 in dfa1.transitions:
        transitions = dfa1.transitions[state1] 
        if state1 in dfa1.final_states:
            transitions.append(dfa2.initial_state)
        nfa.add_state(
            new_states = set(state1),
            transitions = transitions,
            is_final_state = False # Only dfa2 final states are final in concat
        )
    
    for state2 in dfa2.transitions:
        transitions = dfa2.transitions[state2]
        nfa.add_state(
            new_states = set(state2),
            transitions = transitions,
            is_final_state = state2 in dfa2.final_states
        )
    
    concat_dfa = nfa.to_dfa()
    concat_dfa.output_dfa()
